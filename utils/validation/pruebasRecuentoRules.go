...
var PruebaRecuentoRules = map[string]ValidationRule{
	...
	"VolumenDiluyente":       {Regex: regexp.MustCompile(`^[a-zA-Z0-9,]+$`), Message: "El volumen diluyente solo puede contener letras, numeros y comas (,)", Requested: true},
	...
	"NombreRecuento":         {Regex: regexp.MustCompile(`^[a-zA-Z]+$`), Message: "El nombre de recuento solo puede contener letras", Requested: true},
